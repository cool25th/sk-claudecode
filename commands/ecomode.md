...
Run `/sk-claudecode:setup` to set ecomode as your default parallel execution mode.

When set as default, saying "fast" or "parallel" will activate ecomode instead of ultrawork.

## Disabling Ecomode

To completely disable ecomode (prevents all ecomode keyword detection and activation), add to `~/.claude/.skc-config.json`:
...
- `/sk-claudecode:cancel` - Cancel active mode
