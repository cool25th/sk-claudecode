...
1. **First**, spawn Architect to verify your work (ALWAYS pass model explicitly!):
   ```
   Task(subagent_type="sk-claudecode:architect", model="opus", prompt="Verify this implementation is complete: [describe what you did]")
   ```

2. **Wait for Architect's assessment**

3. **If Architect approves**: Run `/sk-claudecode:cancel` to cleanly exit ralph mode
...
- When FULLY complete AND Architect verified: Run `/sk-claudecode:cancel` to exit and clean up state
