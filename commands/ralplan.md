...
Ralplan maintains persistent state in `.skc/ralplan-state.json` to track progress and enable recovery across interruptions:
...
```json
{
  ...
  "iteration": 1,
  "max_iterations": 5,
  "plan_path": ".skc/plans/[feature].md",
...
1. Create `.skc/plans/` directory if it doesn't exist
...
- Planner generates plan directly to `.skc/plans/[feature-name].md`
...
- Signal completion with `PLAN_READY: .skc/plans/[filename].md`
...
- Prepare plan for execution with `/sk-claudecode:ralph` or manual orchestration
...
- Use `/sk-claudecode:cancel` (automatically detects active mode)
- Or manually delete `.skc/ralplan-state.json`
