...
|------------|-----------|---------------|
| `/sk-claudecode:ultraqa --tests` | tests | All test suites pass |
| `/sk-claudecode:ultraqa --build` | build | Build succeeds with exit 0 |
| `/sk-claudecode:ultraqa --lint` | lint | No lint errors |
| `/sk-claudecode:ultraqa --typecheck` | typecheck | No TypeScript errors |
| `/sk-claudecode:ultraqa --custom "pattern"` | custom | Custom success pattern in output |
...
1. **RUN QA**: Execute verification based on goal type
   ...
   - `--interactive`: Use qa-tester for interactive CLI/service testing:
     ```
     Task(subagent_type="sk-claudecode:qa-tester", model="sonnet", prompt="TEST:
...
3. **ARCHITECT DIAGNOSIS**: Spawn architect to analyze failure
   ```
   Task(subagent_type="sk-claudecode:architect", model="opus", prompt="DIAGNOSE FAILURE:
   Goal: [goal type]
...
4. **FIX ISSUES**: Apply architect's recommendations
   ```
   Task(subagent_type="sk-claudecode:executor", model="sonnet", prompt="FIX:
...
Track state in `.skc/ultraqa-state.json`:
```json
{
  "active": true,
  "goal_type": "tests",
...
User can cancel with `/sk-claudecode:cancel` which clears the state file.
