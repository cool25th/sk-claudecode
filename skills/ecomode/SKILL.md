...
Task(subagent_type="sk-claudecode:executor-low", model="haiku", prompt="...")
Task(subagent_type="sk-claudecode:explore", model="haiku", prompt="...")
...
Task(subagent_type="sk-claudecode:architect-medium", model="sonnet", prompt="...")

// AVOID - Only for planning/critique if essential
Task(subagent_type="sk-claudecode:planner", model="opus", prompt="...")
```
...
Ecomode state is tracked in `.skc/state/ecomode-state.json`.

When work is complete, run `/sk-claudecode:cancel` for clean state cleanup.
