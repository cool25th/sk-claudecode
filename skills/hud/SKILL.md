...
| Command | Description |
|---------|-------------|
| `/sk-claudecode:hud` | Show current HUD status (auto-setup if needed) |
| `/sk-claudecode:hud setup` | Install/repair HUD statusline |
| `/sk-claudecode:hud minimal` | Switch to minimal display |
| `/sk-claudecode:hud focused` | Switch to focused display (default) |
| `/sk-claudecode:hud full` | Switch to full display |
| `/sk-claudecode:hud status` | Show detailed HUD status |

## Auto-Setup

When you run `/sk-claudecode:hud` or `/sk-claudecode:hud setup`, the system will automatically:
...
async function main() {
  ...
  if (pluginCacheDir) {
    ...
    console.log("[OMC] Plugin not found. Run: /sk-claudecode:skc-setup");
  }
}
...
1. Run `/sk-claudecode:hud setup` to auto-install and configure
