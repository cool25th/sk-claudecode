...
describe('delegation-enforcer', () => {
  ...
  describe('enforceModel', () => {
    ...
    it('logs warning only when SKC_DEBUG=true', () => {
      const input: AgentInput = {
        description: 'Test task',
        prompt: 'Do something',
    ...
    it('does not log warning when SKC_DEBUG is false', () => {
      const input: AgentInput = {
        description: 'Test task',
        prompt: 'Do something',
        subagent_type: 'executor'
  ...
  describe('processPreToolUse', () => {
    ...
    it('logs warning only when SKC_DEBUG=true and model injected', () => {
      const toolInput: AgentInput = {
        description: 'Test',
        prompt: 'Test',
        subagent_type: 'executor'
  ...
  describe('getModelForAgent', () => {
    it('returns correct model for agent with prefix', () => {
      expect(getModelForAgent('sk-claudecode:executor')).toBe('sonnet');
      expect(getModelForAgent('sk-claudecode:executor-low')).toBe('haiku');
      expect(getModelForAgent('sk-claudecode:architect')).toBe('opus');
    });
