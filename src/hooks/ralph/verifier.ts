 ...
 * 4. If architect approves -> truly complete, use /sk-claudecode:cancel to exit
...
3. **Based on Architect's response:**
   - If APPROVED: Output \`<architect-approved>VERIFIED_COMPLETE</architect-approved>\`, then run \`/sk-claudecode:cancel\` to cleanly exit
...
4. After Architect approves, run \`/sk-claudecode:cancel\` to cleanly exit
