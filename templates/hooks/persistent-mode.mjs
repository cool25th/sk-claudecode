 ...
 *
 * See: https://github.com/Yeachan-Heo/sk-claudecode/issues/213
 */
...
async function main() {
  try {
    ...
    const globalStateDir = join(homedir(), ".skc", "state");
    ...
    if (
      ...
      if (iteration < maxIter) {
        ...
        let reason = `[RALPH LOOP - ITERATION ${iteration + 1}/${maxIter}] Work is NOT done. Continue working.\nWhen FULLY complete (after Architect verification), run /sk-claudecode:cancel to cleanly exit ralph mode and clean up all state files. If cancel fails, retry with /sk-claudecode:cancel --force.\n${ralph.state.prompt ? `Task: ${ralph.state.prompt}` : ""}`;
        if (errorGuidance) {
    ...
    if (
      ...
      if (phase !== "complete") {
        ...
        if (newCount <= 20) {
          ...
          let reason = `[AUTOPILOT - Phase: ${phase}] Autopilot not complete. Continue working. When all phases are complete, run /sk-claudecode:cancel to cleanly exit and clean up state files. If cancel fails, retry with /sk-claudecode:cancel --force.`;
          if (errorGuidance) {
            reason = errorGuidance + reason;
          }
    ...
    if (
      ...
      if (incomplete > 0) {
        ...
        if (newCount <= 20) {
          ...
          let reason = `[ULTRAPILOT] ${incomplete} workers still running. Continue working. When all workers complete, run /sk-claudecode:cancel to cleanly exit and clean up state files. If cancel fails, retry with /sk-claudecode:cancel --force.`;
          if (errorGuidance) {
            reason = errorGuidance + reason;
          }
    ...
    if (
      ...
      if (pending > 0) {
        ...
        if (newCount <= 15) {
          ...
          let reason = `[SWARM ACTIVE] ${pending} tasks remain. Continue working. When all tasks are done, run /sk-claudecode:cancel to cleanly exit and clean up state files. If cancel fails, retry with /sk-claudecode:cancel --force.`;
          if (errorGuidance) {
            reason = errorGuidance + reason;
          }
    ...
    if (
      ...
      if (currentStage < totalStages) {
        ...
        if (newCount <= 15) {
          ...
          let reason = `[PIPELINE - Stage ${currentStage + 1}/${totalStages}] Pipeline not complete. Continue working. When all stages complete, run /sk-claudecode:cancel to cleanly exit and clean up state files. If cancel fails, retry with /sk-claudecode:cancel --force.`;
          if (errorGuidance) {
            reason = errorGuidance + reason;
          }
    ...
    if (
      ...
      if (cycle < maxCycles && !ultraqa.state.all_passing) {
        ...
        let reason = `[ULTRAQA - Cycle ${cycle + 1}/${maxCycles}] Tests not all passing. Continue fixing. When all tests pass, run /sk-claudecode:cancel to cleanly exit and clean up state files. If cancel fails, retry with /sk-claudecode:cancel --force.`;
        if (errorGuidance) {
          reason = errorGuidance + reason;
        }
    ...
    if (
      ...
      if (totalIncomplete > 0) {
        ...
        reason += ` If all work is complete, run /sk-claudecode:cancel to cleanly exit ultrawork mode and clean up state files. If cancel fails, retry with /sk-claudecode:cancel --force. Otherwise, continue working.`;
      } else {
    ...
    if (
      ...
      if (totalIncomplete > 0) {
        ...
        // Only suggest cancel after minimum iterations (guard against no-tasks-created scenario)
        reason += ` If all work is complete, run /sk-claudecode:cancel to cleanly exit ecomode and clean up state files. If cancel fails, retry with /sk-claudecode:cancel --force. Otherwise, continue working.`;
      } else {
        // Early iterations with no tasks yet - just tell LLM to continue
        reason += ` Continue working - create Tasks to track your progress.`;
      }
